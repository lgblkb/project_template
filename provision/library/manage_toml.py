...
import toml, tomlkit, tomlkit.items
...
def run_module():
    ...
    module_args = dict(
        ...
        # mode=dict(type='str', required=False, default='read', choices=['read', 'write'])
    )
    ...
    toml_filepath = module.params['path']
    toml_data = tomlkit.loads(open(toml_filepath).read())
    original_toml_data = tomlkit_to_popo(toml_data).copy()
    user_data = module.params['data']
    if user_data:
        toml_data = merge(toml_data, user_data)
        if tomlkit_to_popo(toml_data) != original_toml_data:
            result['changed'] = True
            with open(toml_filepath, 'w') as file:
                file.write(tomlkit.dumps(toml_data))

    result['meta'] = tomlkit_to_popo(toml_data)
